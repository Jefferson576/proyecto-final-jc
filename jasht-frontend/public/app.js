document.addEventListener('DOMContentLoaded', () => {
    const contenedor = document.getElementById('game-container') || document.getElementById('games-container') || document.querySelector('.games-container');
      ...
      `;
            ...
            if (delBtn) {
                delBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    if (!confirm(`¿Seguro quieres eliminar "${juego.title}"?`))
                        return;
                    try {
                        const res = await fetch(`/games/${juego._id}`, {
                            method: 'DELETE'
                        });
                        if (res.ok) {
    ...
    if (editForm) {
        editForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            if (!juegoActual)
                return alert('No hay juego seleccionado para editar.');
